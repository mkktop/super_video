...
- **智能输出命名**：输出沿用源文件名；目标目录已有同名文件（含源文件本身）时才追加 `_倍率` 后缀区分，批量撞名再退 `_N` 序号——重跑复用同一后缀名，不无限增殖
...
从 [Releases](https://github.com/mkktop/super_video/releases) 下载最新版安装包 `super_video_<版本>_setup.exe`（如 `super_video_0.2.6_setup.exe`，基础包约 240MB），双击安装，无需管理员权限。
...
# 182 通过 + 1 跳过（无 GPU/部分模型缺失时按机器跳过）
```
